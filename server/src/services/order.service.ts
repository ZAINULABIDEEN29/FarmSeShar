...
export const createOrderService = async (
  ...
  // Validate and get product details, link to farmers
  ...
  // Get farmers from products (for now, we'll link to the first product's farmer)
  // In a real scenario, you might need to split orders by farmer
  const firstProduct = products[0];
  const farmerId = firstProduct.farmerId;

  // Create order (orderId will be auto-generated by pre-save hook)
  const order = new Order({
    orderId: `TEMP-${Date.now()}`, // Temporary ID, will be replaced by pre-save hook
  ...
  // Check if quantity reached 0, mark as unavailable
  for (const item of data.items) {
